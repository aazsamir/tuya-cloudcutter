...
if [ $METHOD_DETACH ]; then
	...
	nmcli device set ${WIFI_ADAPTER} managed no; sudo systemctl stop NetworkManager;
	trap "sudo systemctl start NetworkManager; nmcli device set ${WIFI_ADAPTER} managed yes" EXIT  # Set WiFi adapter back to managed when the script exits
